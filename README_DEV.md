...
| `paused` | Boolean | Czy makro jest zapauzowane (Roblox nieaktywny) |
...
| `robloxText` | GuiCtrl | Kontrolka stanu Roblox (✓/✗) |
...
| Makro | AutoPauza | 1 | Auto-pauza gdy Roblox nieaktywny |
...
5. `SetTimer(UpdateTimer, 1000)` + `SetTimer(CheckRoblox, 2000)`
...
- Aktualizuje `robloxText` (zielony/czerwony)
- Jeśli `AutoPauza=1`:
  - Roblox zniknął → `paused := true`, zatrzymuje timery/hotkeys
  - Roblox wrócił → `paused := false`, wznawia timery/hotkeys
...
- **Roblox (RobloxPlayerBeta.exe)** — wykrywany przez `WinExist` do auto-pauzy
